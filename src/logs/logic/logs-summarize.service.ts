...
/**
 * TODO: Segregate prompt into a separate file, such that we can have multiple prompts for different log levels or types.
...
export class LogsSummarizer {
  ...
  // ///////////////
  async buildPromptForAugmentedGeneration(logs: any) {
    const logLines = logs.map((log) => log.pageContent).join('\n');
    const prompt = `
      SYSTEM:
        You are a Backend Log Summarizer. Your audience includes backend engineers, QA, and on-call SREs who may have no prior context.
    ...
    FORMAT (Return ONLY the JSON object below. Do not include code fences, markdown, or explanations.):
        Summary: <plain-English overview>
        Root Cause: <exact exception string>
        Error Context:
          - Class/Module/Function: <name>
